...
- **`--node-modules` anchor**: `@congress/congress-ui` doesn't self-resolve inside its own `node_modules` (it isn't its own dependency). Every converter/resync command must point `--node-modules` at a *consuming* package's node_modules instead — `services/capitol/node_modules` is used throughout (it has the workspace symlink to `packages/congress-ui`, plus `react`/`react-dom`/`react-router-dom`/`@tanstack/react-query`/`@types/react` all resolvable there).
