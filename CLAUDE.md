...
`services/congress/src/events.ts` is a generic, chamber-agnostic append-only event log — any Chamber can publish (`POST /congress/events/publish`, via `chamber-kit`'s `createPublishEvent`) or poll for new entries since a cursor (`GET /congress/events?since=`). Congress never inspects an event's `type`/`payload` or relays it to a chamber by name; it's pure store-and-fan-out, the same spirit as `exhibit_cache`. This log is a switch, not a durable record — retention is short and per-event: a Chamber optionally declares the event types it may publish, and how long each should be kept, in its manifest's `events` field (`manifestEventSchema`, mirroring `widgets`) via an optional `retentionMs`, falling back to Congress's own owner-tunable `eventRetentionMs` setting (a day by default, `CapitolSettings` in `shared-types`/Congress's `settings.ts` — edited from Capitol's Settings page like dark mode, despite the name) when a Chamber doesn't declare one. Congress copies that number onto the row at publish time and prunes on a 5-minute sweep — it's mechanical bookkeeping, not an interpretation of the event, same as it never interprets `payload`. The catalog itself is purely descriptive for another Chamber's own UI to read off the live registry (e.g. a trigger-event picker), not a subscription or a requirement to ever actually fire one. Anything meant to be kept longer than that is Logs Chamber's own job (see below), not Congress's.
