import "dotenv/config";
import { z } from "zod";

// Fields common to every Chamber's env (not Capitol's, whose shape is
// unrelated). PORT and DB_PATH are deliberately excluded - each Chamber
// picks its own port and data path, so those stay in the per-chamber
// schema that extends this one.
export const chamberEnvSchema = z.object({
  HOST: z.string().default("127.0.0.1"),
  CONGRESS_INTERNAL_TOKEN: z.string().min(1, "CONGRESS_INTERNAL_TOKEN must be set"),
  CAPITOL_URL: z.string().url().default("http://127.0.0.1:3000"),
  HEARTBEAT_INTERVAL_MS: z.coerce.number().int().positive().default(30_000),
  NODE_ENV: z.enum(["development", "production", "test"]).default("development"),
});

export function loadEnv<TSchema extends z.ZodTypeAny>(schema: TSchema): z.infer<TSchema> {
  const parsed = schema.safeParse(process.env);
  if (!parsed.success) {
    console.error("Invalid environment configuration:");
    console.error(parsed.error.flatten().fieldErrors);
    throw new Error("Failed to load environment configuration");
  }
  return parsed.data;
}
