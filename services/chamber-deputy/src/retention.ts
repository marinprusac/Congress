...
import { deputySpend, messages, pendingCheckupEvents } from "./db/schema.js";
import { getSettings } from "./settings.js";

// Unlike automation_runs (pruned on insert, capped per-entity), deputy_spend/
...
// actually reach this age - this only matters if checkups have been paused
// (settings.paused) for a very long stretch, so the buffer doesn't grow
...
async function sweep(): Promise<void> {
  ...
  db.delete(deputySpend).where(lt(deputySpend.createdAt, cutoff)).run();
