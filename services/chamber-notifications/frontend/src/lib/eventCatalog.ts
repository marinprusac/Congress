...
// automation editor's trigger-type picker to autocomplete/validate against,
// not the actual event log itself (see eventPoller.ts on this Chamber's own
// backend for that). Nothing here is hardcoded to a specific Chamber name.
export async function fetchEventCatalog(): Promise<EventCatalogEntry[]> {
  const chambers = await fetchRegistry();
  return chambers.flatMap((chamber) => chamber.events.map((event) => ({ ...event, chamber: chamber.name })));
}
