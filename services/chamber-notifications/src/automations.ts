...
function toSummary(row: typeof automations.$inferSelect): AutomationSummary {
  return {
    ...
    actionDedupeKeyTemplate: row.actionDedupeKeyTemplate,
    enabled: row.enabled,
...
export async function createAutomation(input: CreateAutomationRequest): Promise<AutomationDetail> {
  ...
  const inserted = db
    ...
    .values({
      ...
      actionDedupeKeyTemplate: input.actionDedupeKeyTemplate,
...
export async function updateAutomation(id: number, input: UpdateAutomationRequest): Promise<AutomationDetail | null> {
  ...
  const next = {
    ...
    actionUrlTemplate: input.actionUrlTemplate !== undefined ? input.actionUrlTemplate : existing.actionUrlTemplate,
    actionDedupeKeyTemplate: input.actionDedupeKeyTemplate ?? existing.actionDedupeKeyTemplate,
