...
import type { WidgetPlacement } from "@congress/shared-types";
import { fetchLayout, upsertPlacement, deletePlacement } from "@/lib/layoutApi";
...
export function Canvas({ editing, onToggleEditing }: { editing: boolean; onToggleEditing: () => void }) {
  ...
  const layoutQuery = useQuery({
    queryKey: ["congress", "layout", scope],
  ...
  const upsertMutation = useMutation({
    ...
    // for a beat.
    onSuccess: (result) => {
      if (result) return queryClient.invalidateQueries({ queryKey: ["congress", "layout", scope] });
    },
  });

  const deleteMutation = useMutation({
    mutationFn: ({ chamber, widgetId }: { chamber: string; widgetId: string }) => deletePlacement(scope, chamber, widgetId),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["congress", "layout", scope] }),
  });
