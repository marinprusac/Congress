...
import { fetchEventSettings, updateEventSettings, fetchHistory } from "@/lib/logsApi";
import { PayloadView, summarizePayload } from "@/components/PayloadView";
import type { EventHistoryEntry, UpdateEventSettingsRequest } from "@congress/shared-types";
...
export function EventSettingsDetailPage({ eventType, onBack }: { eventType: string; onBack: () => void }) {
  ...
  return (
    <article>
      <button type="button" onClick={onBack} className="mb-4 font-mono text-xs uppercase tracking-wide text-dust hover:text-accent">
        ← All events
      </button>
